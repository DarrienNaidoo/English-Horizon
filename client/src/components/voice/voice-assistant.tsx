...
export default function VoiceAssistant({ className }: VoiceAssistantProps) {
  ...
  return (
    <div className={cn("space-y-6", className)}>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm text-muted-foreground">
                      {SUPPORTED_LANGUAGES.find(l => l.code === result.sourceLanguage)?.name}
                    </span>
                    <span>→</span>
                    <span>{getLanguageFlag(result.targetLanguage)}</span>
                    <span className="text-sm text-muted-foreground">
                      {SUPPORTED_LANGUAGES.find(l => l.code === result.targetLanguage)?.name}
                    </span>
                  </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Quick Text Translation</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <Textarea
